...
// Włączenie self-ping oraz self-trackback
...
function html5blank_header_scripts()
{
    if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
        ...
        wp_enqueue_script( 'wpb_slidepanel', get_template_directory_uri() . '/js/mobile-nav.js', array('jquery'), '1.0.0', true ); // Hamburger
    }
}
...
function html5wp_custom_post($length)
{
  ...
  if ($post->post_type == 'post')
    return 40;
      else if ($post->post_type == 'html5-blank')
    return 30;
}
...
{
    global $post;
    return '... <a class="link font-bold mt-4" href="' . get_permalink($post->ID) . '">' . __('Zobacz więcej', 'html5blank') . '</a>';
}
